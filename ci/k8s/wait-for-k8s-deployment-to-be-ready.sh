...
while [ $(( $(date +%s) - 300 )) -lt ${starttime} ]; do
    ...
    consumer_status=`kubectl get pods -l "akvo-authz-version=$TRAVIS_COMMIT,run=akvo-authz" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}'`
...
# Another possibility could be to check that the service is pointing just to the new containers.
    old_consumer_status=`kubectl get pods -l "akvo-authz-version!=$TRAVIS_COMMIT,run=akvo-authz" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}'`
...
kubectl get pods -l "run=akvo-authz-version" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}'
