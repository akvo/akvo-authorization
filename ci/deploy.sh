...
if [[ "${TRAVIS_TAG:-}" =~ promote-.* ]]; then
    ...
    UNILOG_CONSUMER_POD_CPU_LIMITS="2000m"
    UNILOG_CONSUMER_POD_MEM_REQUESTS="800Mi"
    UNILOG_CONSUMER_POD_MEM_LIMITS="800Mi"
else
...
sed -e "s/\$TRAVIS_COMMIT/$TRAVIS_COMMIT/" \
  -e "s/\${ENVIRONMENT}/${ENVIRONMENT}/" \
  -e "s/\${UNILOG_CONSUMER_POD_CPU_REQUESTS}/${POD_CPU_REQUESTS}/" \
  -e "s/\${UNILOG_CONSUMER_POD_MEM_REQUESTS}/${POD_MEM_REQUESTS}/" \
  -e "s/\${UNILOG_CONSUMER_POD_CPU_LIMITS}/${POD_CPU_LIMITS}/" \
  -e "s/\${UNILOG_CONSUMER_POD_MEM_LIMITS}/${POD_MEM_LIMITS}/" \
